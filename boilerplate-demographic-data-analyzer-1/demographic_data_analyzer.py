...
def calculate_demographic_data(print_data=True):
    ...
    print(df.loc[df['sex'] == "Male"]['age'].sum())
    average_age_men = round(df.loc[df['sex'] == "Male"]['age'].mean(),1)

    # What is the percentage of people who have a Bachelor's degree?
    print(len(df.loc[df['education'] == "Bachelors"]))
    print(len(df['education']))
    print(len(df.loc[df['education'] == "Bachelors"])/len(df['education']))
    ...
    print(df['education'])
    print(df['salary'])
    print(len(df.loc[(df['education-num'] > 12) & (df['salary'] == ">50K")]))
    print(len(df.loc[df['education-num'] > 12]))
    print(len(df.loc[(df['education-num'] > 12) & (df['salary'] == ">50K")])/len(df.loc[df['education-num'] > 12]))
    ...
    print(df.loc[(df['hours-per-week'] == df['hours-per-week'].min()) & (df['salary'] == ">50K")])
    num_min_workers = len(df.loc[df['hours-per-week'] == df['hours-per-week'].min()])

    rich_percentage = len(df.loc[(df['hours-per-week'] == df['hours-per-week'].min()) & (df['salary'] == ">50K")])/num_min_workers * 100

    # What country has the highest percentage of people that earn >50K?
    p = (df[df['salary'] == ">50K"]['native-country'].value_counts() \
                                / df['native-country'].value_counts() * 100).sort_values(ascending=False)
    print(p)
    print(df.loc[df['salary'] == ">50K"]['native-country'].mode)
